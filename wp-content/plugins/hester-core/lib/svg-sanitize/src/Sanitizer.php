...
namespace HesterCoreVendor\enshrined\svgSanitize;

use HesterCoreVendor\enshrined\svgSanitize\data\AllowedAttributes;
use HesterCoreVendor\enshrined\svgSanitize\data\AllowedTags;
use HesterCoreVendor\enshrined\svgSanitize\data\AttributeInterface;
use HesterCoreVendor\enshrined\svgSanitize\data\TagInterface;
use HesterCoreVendor\enshrined\svgSanitize\data\XPath;
use HesterCoreVendor\enshrined\svgSanitize\ElementReference\Resolver;
/**
 * Class Sanitizer
 *
			...
			if ( $currentElement instanceof \DOMElement ) {
				...
				if ( strtolower( $currentElement->tagName ) === 'font' ) {
					...
					for ( $x = $currentElement->attributes->length - 1; $x >= 0; $x-- ) {
						...
						if ( in_array( strtolower( $attrName ), array( 'face', 'color', 'size' ) ) ) {
