...
<?php endif; ?>
	...
	public function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			$this->create_taxonomy( $term['term_taxonomy'] );
	...
	public function create_taxonomy($taxonomy) {
		global $wpdb;
		if ( strstr( $taxonomy, 'pa_' ) && ! taxonomy_exists( $taxonomy ) && class_exists( 'WooCommerce' ) ) {
			$attribute_name = wc_sanitize_taxonomy_name( str_replace( 'pa_', '', $taxonomy ) );
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		if('_elementor_template_type' === $key && 'kit' === $meta['value']){
		...
		if( $key ==='wcboost_variation_swatches' ){
			$data = [];

			foreach ( $value as $attribute_slug => $settings ) {
				$data[ $attribute_slug ] = [
					'type'        => $settings['type'],
					'shape'       => $settings['shape'],
					'size'        => $settings['size'],
					'custom_size' => $settings['custom_size'],
					'swatches'    => [],
				];

				foreach ( $settings['swatches'] as $term_id => $swatches ) {
					$term_id = $this->processed_terms[ intval( $term_id ) ];
					$data[ $attribute_slug ]['swatches'][ $term_id ] = array_map( 'sanitize_text_field', $swatches );
				}
			}
			
			update_post_meta( $post_id, wp_slash( $key ), wp_slash( $data ) );
			do_action( 'import_post_meta', $post_id, $key, $data );
		}else{
	...
	protected function process_post_comment_meta( $comment_id, $meta ) {
		...
		$value = $this->maybe_unserialize( $meta['value'] );
		add_comment_meta( $comment_id, wp_slash( $meta['key'] ), wp_slash( $value ) );
}
	/**
	...
	public function process_menu_item( $item ) {
		...
		if ( $id && ! is_wp_error( $id ) ) {
            if(isset($_menu_item_mega)) {
                update_post_meta($id, '_menu_item_mega', unserialize($_menu_item_mega));
            }
            if(isset($_menu_item_mega_grid)) {
				
				$_menu_item_mega_grid = unserialize($_menu_item_mega_grid);
		
				// Ensure it's an array and has items
				if (is_array($_menu_item_mega_grid) && isset($_menu_item_mega_grid['items'])) {
					foreach ($_menu_item_mega_grid['items'] as $key=>&$menu_item) {
						if (empty($menu_item['parent_cat'])) continue;
						
						$parent_cat_id = intval($menu_item['parent_cat']);
						if (isset($this->processed_terms[$parent_cat_id])) {
							$menu_item['parent_cat'] = (int) $this->processed_terms[$parent_cat_id];
						}
					}
					unset($menu_item); // Break the reference
				}
				update_post_meta($id, '_menu_item_mega_grid', $_menu_item_mega_grid);
            }
            if(isset($_menu_item_mega_tab)) {
                update_post_meta($id, '_menu_item_mega_tab', unserialize($_menu_item_mega_tab));
            }
